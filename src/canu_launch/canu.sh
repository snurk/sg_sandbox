...
mkdir -p $out_dir
$CANU_BIN/canu -p asm -d $out_dir -assemble canuIterationMax=1 stopOnLowCoverage=0 minInputCoverage=0 maxInputCoverage=10000 batMemory=200 genomeSize='3.1g' gridOptionsUTGOVL='--time=12:00:00' gridOptionsmeryl='--mem-per-cpu=4g' gridOptions='--time=48:00:00' ovlThreads=32 utgOvlRefBlockLength=10000000000 ovlMerThreshold=20000 minReadLength=4000 minOverlapLength=1000 batOptions="-covgaptype uncovered -D symmetricoverlaps -stop bestedges -ef 1e-5 -readlen 1-$MAX_BOGART_LEN" stopAfter=unitig -pacbio-hifi $reads "${@:3}"
