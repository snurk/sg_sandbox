...
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 <chr_name> <reference> <fragments> <out_dir> [max gap size (default: 10000)]"
    exit 1
fi
...
minimap2 -a -H -t 8 -x asm20 -I 8G $ref $contigs | samtools view -b -F 4 > $align_prefix.bam
...
$root/gap_patch.py --max-gap $max_gap --reliable-len 10000 --max-trim 200 $out_dir/contig.info $align_prefix.cout 2> $out_dir/patch.log | sort | uniq | sed 's/^.*: //g' > $out_dir/patch.bed
